...
namespace Common.Logging.NLog45
{
	...
	public class CommonLoggingTarget : TargetWithLayout
	{
		...
		static CommonLoggingTarget()
		{
			...
			{
				[global::NLog.LogLevel.Trace] = delegate (ILog log, MessageFormatter msg, Exception ex) { log.Trace(delegate (FormatMessageHandler m) { m(msg()); }, ex); },
				[global::NLog.LogLevel.Debug] = delegate (ILog log, MessageFormatter msg, Exception ex) { log.Debug(delegate (FormatMessageHandler m) { m(msg()); }, ex); },
				[global::NLog.LogLevel.Info] = delegate (ILog log, MessageFormatter msg, Exception ex) { log.Info(delegate (FormatMessageHandler m) { m(msg()); }, ex); },
				[global::NLog.LogLevel.Warn] = delegate (ILog log, MessageFormatter msg, Exception ex) { log.Warn(delegate (FormatMessageHandler m) { m(msg()); }, ex); },
				[global::NLog.LogLevel.Error] = delegate (ILog log, MessageFormatter msg, Exception ex) { log.Error(delegate (FormatMessageHandler m) { m(msg()); }, ex); },
				[global::NLog.LogLevel.Fatal] = delegate (ILog log, MessageFormatter msg, Exception ex) { log.Fatal(delegate (FormatMessageHandler m) { m(msg()); }, ex); },
				[global::NLog.LogLevel.Off] = delegate (ILog log, MessageFormatter msg, Exception ex) { }
			};
		}
		...
		{
			ArgUtils.AssertNotNull(nameof(layout), layout);
			this.Layout = layout;
		}
